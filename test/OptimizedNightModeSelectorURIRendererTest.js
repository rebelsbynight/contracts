...
beforeEach(async () => {
  ...
  NightModeSelectorURIRenderer = await ethers.getContractFactory("OptimizedNightModeSelectorURIRenderer");
...
describe("OptimizedNightModeSelectorURIRenderer Unit Tests", () => {
  ...
  it("Should revert if non-owner attempts to update night mode", async () => {
    ...
    await expect(nightModeSelectorURIRenderer.connect(addr1).setNightMode(1)).to.be.revertedWith("Not token owner");
  });
  ...
  it("Should set night mode correctly for token owner", async () => {
    ...
    await nightModeSelectorURIRenderer.connect(owner).setNightMode(1);
    expect(await nightModeSelectorURIRenderer.getNightMode(1)).to.be.true;

    await nightModeSelectorURIRenderer.connect(owner).unsetNightMode(1);
  ...
  it("Should revert if trying to set night mode for nonexistent token", async () => {
    await expect(nightModeSelectorURIRenderer.connect(owner).setNightMode(9999)).to.be.revertedWith("ERC721: invalid token ID");
  });
});
